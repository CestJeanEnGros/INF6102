...
class Instance:
    ...
    def neighs(self, node: Node) -> Iterable[Edge]:
        """
            Return all node neighbors
        """
        neighs = list(filter(lambda e: node in e.idx(), self.edges))
...
class FloydWarshall:
    ...
    def get_shortest_path(self, start_idx, end_idx, instance: Instance):
        """ Reconstruit le plus court chemin entre deux noeuds donnés.
    Args:
        start_idx (int): L'id du noeud de départ
        end_idx (int): L'id du noeud d'arrivé
        instance (Instance): L'instance du problème

    Returns:
        (list): La liste des noeuds composant le plus court chemin
    """
    ...
    def get_distance(self, start_idx, end_idx):
        """ Retourne la distance minimale entre deux noeuds donnés.
    Args:
        start_idx (int): L'id du noeud de départ
        end_idx (int): L'id du noeud d'arrivé

    Returns:
        (int): Le cout du plus court chemin
    """
        return self.dist[start_idx-1][end_idx-1]
    
    def get_real_distance_and_path(self, start_idx, end_idx, tree, instance, branch):
        """ Retourne le cout et le chmin minimal réel, en prenant compte des arretes déjà présentes dans la solution
    Args:
        start_idx (int): L'id du noeud de départ
        end_idx (int): L'id du noeud d'arrivé
        tree (Tree): Arbre de la solution actuelle
        insatnce (Instance): Instance du problème
        branch (list): Liste des noeuds composant la branche de l'arbre qui va être supprimer 

    Returns:
        cost (int): Le cout du plus court chemin
        real_path (list): 
    """
        path = self.get_shortest_path(end_idx, start_idx, instance)
        start_path_idx = path[-1].idx()
        end_path_idx = path[0].idx()

        for i, n in enumerate(path):
            n_id = n.idx()
            n_n_id = path[i+1].idx()
            if n_n_id not in tree.nodes or not tree.is_ancestor(end_path_idx, n_n_id):
                end_path_idx = n_id
                break
        ...
        real_path = self.get_shortest_path(end_path_idx, start_path_idx, instance)
        cost = self.get_distance(end_path_idx, start_path_idx)
