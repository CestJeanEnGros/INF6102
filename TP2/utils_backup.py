...
class Instance:
    def __init__(self,in_file: str):
        ...
        with open(self.filepath) as f:
            ...
            self.edges = set()
            # Rajout des arrêtes sous forme de dictionnaire afin d'en faciliter l'accès
            ...
            for line in filter(lambda line : line[0] == "E", lines[1:]):
                ...
                # Rajout des arrêtes sous forme de dictionnaire afin d'en faciliter l'accès
                self.edges_dict[frozenset({n1, n2})] = Edge(self.nodes[n1], self.nodes[n2], cost)
...
class FloydWarshall:
    def __init__(self, instance: Instance):
        """
        Initialisation de l'algorithme de Floyd-Warshall avec les sommets et les arêtes.

        :param nodes: Liste des nœuds du graphe (objets de la classe Node).
        :param edges: Liste des arêtes du graphe (objets de la classe Edge).
        """
        self.dist = np.full((instance.N, instance.N), np.inf)
        self.next_node = np.full((instance.N, instance.N), None)

        # Initialisation des matrices dist et next_node
        for edge in instance.edges:
            n1, n2 = edge.idx()
            n1_id, n2_id = n1.idx(), n2.idx()
            self.dist[n1_id-1][n2_id-1] = edge.cost()
            self.dist[n2_id-1][n1_id-1] = edge.cost()  
            self.next_node[n1_id-1][n2_id-1] = n2_id
            self.next_node[n2_id-1][n1_id-1] = n1_id

        # Les distances à soi-même sont 0
        for i in range(instance.N):
            self.dist[i][i] = 0
            self.next_node[i][i] = i

    def floyd_warshall(self, instance: Instance):
        """
        Exécute l'algorithme de Floyd-Warshall pour calculer les plus courts chemins entre toutes les paires de sommets.
        """
        for k in tqdm(range(instance.N)):
            for i in range(instance.N):
    ...
    def get_shortest_path(self, start_idx, end_idx, instance: Instance):
        """
        Reconstruit le plus court chemin entre deux sommets donnés.

        :param start_idx: L'index du sommet de départ.
        :param end_idx: L'index du sommet d'arrivée.
        :return: Liste des nœuds du plus court chemin.
        """

        if self.next_node[start_idx-1][end_idx-1] is None:
            return []  # Aucun chemin n'existe

        path = [start_idx]
        while start_idx != end_idx:
            start_idx = self.next_node[start_idx-1][end_idx-1]
            path.append(start_idx)
        return [instance.nodes[i] for i in path]

    def get_distance(self, start_idx, end_idx):
        """
        Retourne la distance minimale entre deux sommets donnés.

        :param start_idx: L'index du sommet de départ.
        :param end_idx: L'index du sommet d'arrivée.
        :return: La distance minimale entre les deux sommets.
        """
        return self.dist[start_idx-1][end_idx-1]
    
    def get_real_distance_and_path(self, start_idx, end_idx, tree, instance, branch):
        path = self.get_shortest_path(end_idx, start_idx, instance)
        start_path_idx = path[-1].idx()

        for i, n in enumerate(path[1:]):
            n_id = n.idx()
            if n_id in tree.nodes and n_id not in branch and not tree.is_ancestor(path[0].idx(), n_id):
                start_path_idx = n_id
                break
        
        real_path = self.get_shortest_path(end_idx, start_path_idx, instance)

        cost_corr = 0
        for i in range(len(real_path) - 1):
            n_i_id = real_path[i].idx()
            n_n_id = real_path[i+1].idx()
            if n_n_id in tree.nodes and n_i_id in tree.nodes and ((tree.nodes[n_i_id].parent == n_n_id and n_i_id not in branch) or tree.nodes[n_n_id].parent == n_i_id):
                cost_corr += instance.edges_dict[frozenset({n_i_id, n_n_id})].cost()
        cost = self.get_distance(real_path[0].idx(), real_path[-1].idx()) - cost_corr
        
        # print(f"real_path {real_path}")
        # for i in range(len(real_path)):
        #     if real_path[i].idx() in tree.nodes:
        #         n = tree.nodes[real_path[i].idx()]
        #         print(n.id, n.child, n.parent)
        #     else:
        #         print(f"{real_path[i].idx()} pas dans l'arbre")
        # print("cout des arretes du real_path")
        # for i in range(len(real_path)-1):
        #     print(instance.edges_dict[frozenset({real_path[i].idx(), real_path[i+1].idx()})].cost())
        # print("comparaison des couts")
        # print(cost, )
